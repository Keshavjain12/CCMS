...
function resolveRecipients(newStatus, complaint, actorUser) {
  ...
  (matrix.alsoNotifyRoles || []).forEach((roleName) => {
    const role = md.roles.find((r) => r.roleName === roleName);
    if (role) {
      md.users
        .filter((u) => u.roleId === role.roleId && u.active)
        .forEach((u) => emails.add(u.email));
...
const CUSTOMER_EVENTS = {
  ...
  clarification: {
    ...
    subject: (c) => `[CCMS] We need more information — ${c.complaintNo}`,
    intro:   "Our team needs some additional information to continue processing your complaint.",
    message: "Please respond with the requested details at your earliest convenience so we can proceed without delay.",
  },
  rejected: {
    statusLabel: "Returned for Revision",
    subject: (c) => `[CCMS] Action needed on complaint ${c.complaintNo}`,
    intro:   "Your complaint has been returned by our review team for revision.",
    message: "Our team may contact you for additional details. Processing will resume once the noted points are addressed.",
  },
  auto_closed: {
    statusLabel: "Auto-Closed (No Response)",
    subject: (c) => `[CCMS] Complaint ${c.complaintNo} has been closed`,
    intro:   "Your complaint has been automatically closed as it was awaiting a response beyond the allowed period.",
    message: "If this was closed in error or you still need assistance, please raise a new complaint referencing this number.",
  },
};
...
const STATUS_TO_CUSTOMER_EVENT = {
  ...
  Closed:               "closed",
  Clarification_Sought: "clarification",
