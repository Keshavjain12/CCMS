...
router.post("/:complaintNo/action", async (req, res) => {
  ...
  // Reject moves the complaint back to the previous status (never a "Rejected"
  // status), so notify the customer explicitly that it was returned for revision.
  if (action === "reject") {
    notify.sendCustomerNotification({ complaint: updatedComplaint, event: "rejected" })
      .catch((err) => console.error("[NOTIFY] Customer rejection error:", err.message));
  }
