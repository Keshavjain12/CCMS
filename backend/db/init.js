...
const { dbConfig, isManaged } = require("./pgConfig");
...
async function main() {
  ...
  if (!managed && (!process.env.PGPASSWORD ||
      process.env.PGPASSWORD === "CHANGE_ME" ||
      process.env.PGPASSWORD === "your_postgres_password")) {
    fail("PGPASSWORD is not set in backend/.env",
         "Copy .env.example to .env and fill in your PostgreSQL password.");
  }

  if (managed) {
    // Hosted database (DATABASE_URL): it already exists and we cannot connect
    // to the "postgres" maintenance database or run CREATE DATABASE. Use it as-is.
    console.log("ℹ️  DATABASE_URL set — using the managed database as-is (no CREATE DATABASE).");
  } else {
    // Local server: create the database first via the maintenance connection.
    const admin = new Client(dbConfig({ database: "postgres" }));
    try {
      await admin.connect();
    } catch (err) {
      fail(`Cannot reach PostgreSQL at ${process.env.PGHOST || "localhost"}:${process.env.PGPORT || "5432"} — ${err.message}`,
  ...
  try {
    ...
    fail(`Cannot connect to the CCMS database — ${err.message}`,
         managed ? "Check DATABASE_URL, and that SSL is allowed (see PGSSL* settings in .env.example)."
