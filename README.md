...
| **Email Notifications** | Customers emailed at every milestone (registration → closure) plus exceptions; staff routed the right action per stage; full trail persisted to the DB and viewable in-app |
...
### Email notifications — the trail, in-app
Every email the system sends is also recorded and shown here: a per-complaint
**Emails** tab and a global **Notifications** page, grouped into collapsible
sections. Nothing is lost to a black box — you can see exactly who was told what,
and when.

![Notifications](docs/screenshots/07-emails.png)

### Dark & light themes
The entire UI ships in both. Toggle it from the top bar; the choice persists per
browser, and every screen — dashboards, tables, forms, the workflow tracker — is
styled for each. Dark theme shown below.
...
As a complaint moves through its lifecycle the system keeps everyone informed by
email — **customers** on the outside, **staff** on the inside. Mail is sent over
SMTP (`nodemailer`), and every message is **persisted to PostgreSQL**, so the
trail survives restarts and is viewable in-app: a per-complaint **Emails** tab
and a global **Notifications** page (grouped into collapsible sections).

### Customer emails

The registered customer is emailed at each milestone, once per complaint
(deduplicated — a reject → re-approve won't double-send):

| Trigger | Email |
|---|---|
| Complaint created | Acknowledgement — Complaint No, invoice, date, status, tracking note |
| Technical review complete (→ QC) | "Technical Review Complete" |
| QC review complete (→ CAPA) | "Quality Review Complete" |
| MD approval reached | "Management Review" |
| Finance processing | "Approved — Credit Note in progress" |
| Closed | Final — resolved, with Credit Note number |
| Clarification requested | "We need more information" |
| Returned / rejected | "Action needed — returned for revision" |
| Auto-closed (SLA) | "Complaint closed — no response" |

### Staff emails

At every stage the **next responsible role** is emailed the action it must take
— TS Head, QC Manager (plus the QC Analyst / TS Officer who do the hands-on
work), Operations, Marketing, MD, Sales/KAM, Finance — via a role-based routing
matrix. Reject / clarification / closure notify the relevant stakeholders too.

### Configuration (`backend/.env`)

| Var | Purpose |
|---|---|
| `NOTIFY_MODE` | `mock` (logs only, nothing sent) or `live` (real emails via SMTP) |
| `SMTP_HOST` / `SMTP_PORT` | SMTP server, e.g. Gmail `smtp.gmail.com:587` or Brevo `smtp-relay.brevo.com:587` |
| `SMTP_USER` / `SMTP_PASS` | SMTP login + key / app-password |
| `SMTP_FROM` | Verified sender address |

In `mock` mode nothing is transmitted — messages are recorded and shown under
`GET /api/notifications` and the in-app Notifications page, ideal for development
and demos. Flip to `live` with valid SMTP credentials to send real email.
...
| `PGPASSWORD` | — | Your PostgreSQL password (local dev) — never commit it |
| `PG_POOL_MAX` | 10 | Max pooled connections |
| `DATABASE_URL` | — | Hosted Postgres connection string. **When set it overrides the `PG*` vars and enables TLS.** |
| `PGSSL` / `PGSSLMODE` | — | Force TLS on/off for the `PG*` path (auto-on with `DATABASE_URL`) |
| `NODE_ENV` | development | `production` = Secure cookies, HSTS, HTTPS enforcement, error masking |
| `JWT_SECRET` | — | **Required in production.** Long random token-signing secret; server refuses to boot on a weak one |
| `JWT_EXPIRES` | 8h | Token lifetime |
| `CORS_ORIGIN` | http://localhost:5173 | Comma-separated allow-list of frontend origins |
| `COOKIE_SAMESITE` | lax | `none` for a cross-domain frontend/backend (needs HTTPS) |
| `TRUST_PROXY` | false | `true` behind a platform proxy (Render/Heroku/Nginx) |
...
| `NOTIFY_MODE` | mock | `live` sends real email via SMTP |
| `SMTP_HOST` … `SMTP_FROM` | — | SMTP host/port/user/pass/from for `live` mode |
