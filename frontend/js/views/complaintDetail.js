...
CCMS.views.complaintDetail = async function (mount, params) {
  ...
  async function sampleForm(c) {
    ...
    const mode = el("select.input", {}, ["Courier", "Hand Delivery", "Field Pickup"]
      .map((x) => el("option", { value: x, text: x })));
  ...
  function sampleUpdateForm(c, s) {
    ...
    const result = el("select.input", {}, ["", "Pass", "Fail", "Inconclusive"].map((x) =>
      el("option", { value: x, selected: x === (s.testResult || "") ? "selected" : null, text: x || "— not tested —" })));
