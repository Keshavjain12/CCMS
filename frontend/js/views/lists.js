...
CCMS.views.notifications = async function (mount) {
  ...
  try {
    ...
    // Group emails by complaint, most recent activity first (items arrive newest-first).
    const groups = new Map();
    items.forEach((n) => {
      const key = n.complaintNo || "System";
    ...
    function notifItem(n) {
      ...
      return el("div.notif-item", {}, [
        el("div.sub-main", {}, [
          n.channel ? CCMS.ui.pill(isCust ? "Customer" : "Team", isCust ? "pill-ok" : "pill-warn") : null,
    ...
    groups.forEach((list, key) => {
      const body = el("div.notif-group-body");
      if (key !== "System") body.appendChild(el("a.link.sm", { href: "#/complaints/" + key, text: "Open " + key + " →" }));
      list.forEach((n) => body.appendChild(notifItem(n)));
      body.style.display = "none";

      const caret = el("span.notif-group-caret", { text: "▸" });
      const head = el("button.notif-group-head", { type: "button", title: "Show / hide emails for " + key }, [
        caret,
        el("span.notif-group-title", { text: key }),
        el("span.notif-group-count", { text: list.length + " email" + (list.length === 1 ? "" : "s") }),
      ]);
      let open = false;
      ...
      card.appendChild(el("div.notif-group", {}, [head, body]));
    });
